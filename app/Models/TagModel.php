...
class TagModel extends BaseModel{
    ...
    public function addTag(string $tagName, string $tagSlug, bool $isPublished): Tag|bool{
        $sql = "INSERT INTO tag (name, slug, is_published) 
                VALUES ('{$tagName}', '{$tagSlug}', " . ((int) $isPublished) . ")";
        $res = $this->database->query($sql);
    ...
    public function updateTag(int $id, string $name, string $slug, bool $isPublished): Tag|bool{
        $sql = "UPDATE tag SET name = '{$name}', slug = '{$slug}', is_published = " 
                . ((int) $isPublished) . " WHERE id = {$id}";
        if(!$this->database->query($sql)) return false;
