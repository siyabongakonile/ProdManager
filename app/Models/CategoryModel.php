...
class CategoryModel extends BaseModel{
    ...
    public function addCategory(string $name, string $slug, ?int $parent, bool $isPublished = false): Category|bool{
        ...
        $dbIsPublished = $this->database->convertBool($isPublished);
        $sql = "INSERT INTO category (name, slug, parent_id, is_published) 
                VALUES ('{$name}', '{$slug}', {$dbParent}, {$dbIsPublished})";
        echo $sql;
        $res = $this->database->query($sql);
    ...
    public function updateCategory(int $id, string $name, string $slug, ?int $parent, bool $isPublished): bool{
        ...
        $sql = "UPDATE category SET 
                name='$name', slug='$slug', parent_id=$dbParent, 
                is_published=" . ((int) $isPublished) . " WHERE id=$id";
